...
void counter(vector<int> &nums, int start1, int end1, int start2, int end2, vector<int> &output, int &count) {
    if(start1>end1) {
    ...
    if(nums.at(start1)>=nums.at(start2)) {
        count++;
        output.push_back(nums.at(start2));
        counter(nums, start1, end1, start2+1, end2, output, count);
    }
    else if(nums.at(start1)<=nums.at(start2)) {
        output.push_back(nums.at(start1));
        counter(nums, start1+1, end1, start2, end2, output, count);
    }
}

void inversionCount(vector<int> &nums, int start, int end, int &count) {
    if(start>=end) return;
    else {
        int mid = start+(end-start)/2;
        inversionCount(nums, start, mid, count);
        inversionCount(nums, mid+1, end, count);
        vector<int> output;
        counter(nums, start, mid, mid+1, end, output, count);
...
vector<int> solve() {
    ...
    inversionCount(nums, 0, nums.size()-1, count);
