...
void quickSort(vector<int> &nums, int start, int end, int pivot) {
    if(nums.size()<=1) {
        return;
    }
    if(nums.size()==2) {
        if(nums.at(0)>nums.at(1)) swap(nums, 0, 1);
        return;
    }
    if(start>=end) {
        return;
        ...
        quickSort(nums, start, pivot-2, pivot-1);
        quickSort(nums, pivot+1, end, end+1);
    }
}
...
vector<int> solve() {
    ...
    quickSort(nums, 0, nums.size()-2, nums.size()-1);
