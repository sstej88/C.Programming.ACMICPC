...
int maxHeight(vector< vector<int> > &cuboids) {
    ...
    while(!pq.empty()) {
        ...
        cout<<endl;
        ins.push_back(pq.top());
        pq.pop();
    }
    cout<<endl;

    int *dp = new int[ins.size()];
    dp[0] = ins.at(0).at(0);
    for(int i=1; i<ins.size(); i++) {
        dp[i] = ins.at(i).at(0);
        for(int j=0; j<i; j++) {
            if(ins.at(j).at(1)<=ins.at(i).at(1) && ins.at(j).at(2)<=ins.at(i).at(2)) {
                dp[i] += ins.at(j).at(0);
            }
            else if(ins.at(j).at(2)<=ins.at(i).at(1) && ins.at(j).at(1)<=ins.at(i).at(2)) {
                dp[i] += ins.at(j).at(0);
            }
        }
    }
