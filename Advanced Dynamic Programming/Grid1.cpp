...
int dfs(vector<vector<int> > nums, int i, int j, vector<vector<bool> > visited, int n, int m) {
    if(i==n-1 && j==m-1) {
        return 1;
    }
    if(i<n && j<m) {
        ...
        if(!visited[i][j]) {
            visited[i][j] = true;
            sum+=dfs(nums, i, j+1, visited, n, m);
            sum+=dfs(nums, i+1, j, visited, n, m);
        }
        return sum;    
    }
    else {
...
vector<int> solve() {
    ...
    while(n--) {
        int t = m;
        vector<int> v;
        vector<bool> subVisited;
        vector<int> dpArray;
        while(t--) {
            cin>>e;
            v.push_back(e);
            if(e==0) {
                subVisited.push_back(true);
            }
            else {
                subVisited.push_back(false);
            }
            dpArray.push_back(INT_MAX);
        }
        input.push_back(v);
        visited.push_back(subVisited);
        dp.push_back(dpArray);
    }

    cout<<dfs(input, 0, 0, visited, bN, bM)<<endl;
