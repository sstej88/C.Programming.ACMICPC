...
void vacation(vector< vector<int> > activities, int i, int dp[][3]) {
    if(i==activities.size()) {
        return;
    }
    dp[i][0] = max(dp[i-1][0]+activities[i][1], dp[i-1][0]+activities[i][2]);
    dp[i][1] = max(dp[i-1][1]+activities[i][0], dp[i-1][1]+activities[i][2]);
    dp[i][2] = max(dp[i-1][2]+activities[i][0], dp[i-1][2]+activities[i][1]);
    vacation(activities, i+1, dp);
}
