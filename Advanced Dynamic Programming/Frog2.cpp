...
int frog(vector<int> nums, int n, vector<int> &dp, int k) {
    if(n==nums.size()-1) {
        return 0;
    }
    if(n>nums.size()-1) {
    ...
    if(k==1) {
        if(nums.at(n)>=nums.at(n+1)) {
            pq.push(nums.at(n)-nums.at(n+1)+frog(nums, n+1, dp, k));
        }
        else {
            pq.push(nums.at(n+1)-nums.at(n)+frog(nums, n+1, dp, k));
        }
    }
    else {
        for(int j=n+1; j<=n+k; j++) {
            if(n+j<nums.size()) {
                if(nums.at(n)>=nums.at(n+j)) {
                    pq.push(nums.at(n)-nums.at(n+j)+frog(nums, n+j, dp, k));
                }
                else {
                    pq.push(nums.at(n+j)-nums.at(n)+frog(nums, n+j, dp, k));
                }
            }
        }
    }
