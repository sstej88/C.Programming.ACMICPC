...
int frog(vector<int> nums, int n, vector<int> &dp) {
    if(n==nums.size()-1) {
        return 0;
    }
    if(n>nums.size()-1) {
    ...
    if(nums.at(n)>=nums.at(n+1)) {
        pq.push(nums.at(n)-nums.at(n+1)+frog(nums, n+1, dp));
    }
    else {
        pq.push(nums.at(n+1)-nums.at(n)+frog(nums, n+1, dp));
    }
    if(n+2<nums.size()) {
        if(nums.at(n)>=nums.at(n+2)) {
            pq.push(nums.at(n)-nums.at(n+2)+frog(nums, n+2, dp));
        }
        else {
            pq.push(nums.at(n+2)-nums.at(n)+frog(nums, n+2, dp));
        }
    }
    dp[n] = pq.top();
