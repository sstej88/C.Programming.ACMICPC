...
int findingLargestRoute(list<int> *l, int numberOfNodes, unordered_map<int, bool> &visited, int i, unordered_map<int, int> &dp) {
    ...
    for(auto x: l[i]) {
        pq.push(findingLargestRoute(l, numberOfNodes, visited, x, dp));
    }
    return dp[i] = pq.top()+1;
}
...
vector<int> solve() {
    ...
    priority_queue<int> pq;
    for(int i=1; i<=n; i++) {
        pq.push(findingLargestRoute(l, numberOfNodes, visited, i, dp));
    }
    cout<<pq.top()<<endl;
