...
void merge(vector<int> nums1, vector<int> nums2, vector<int> &output, int i, int j, int &counter, int &sum) {
    if(i==nums1.size()) {
        while(j!=nums2.size()) {
            sum+=nums2.at(j);
    ...
    if(j==nums2.size()) {
        ...
        if(nums1.at(i)>=nums2.at(j)) {
            sum+=nums2.at(j);
            output.push_back(nums2.at(j));
            merge(nums1, nums2, output, i, j+1, counter, sum);
        }
        else if(nums1.at(i)<nums2.at(j)) {
            sum+=nums1.at(i);
            output.push_back(nums1.at(i));
            merge(nums1, nums2, output, i+1, j, counter, sum);
        }
    }
}
...
vector<int> solve() {
    ...
    int counter = 0;
    int sum = 0;
    merge(nums1, nums2, output, 0, 0, counter, sum);

    if(output.size()&1) {
        cout<<output.at(output.size()/2)<<endl;
    }
    else {
        int mid = output.size() / 2;
        cout<<(output.at(mid-1) + output.at(mid))/2.0<<endl;
    }
