...
void lowerBoundSearch(vector<int> nums, int &lowerBound, int target) {
    int start = 0;
    ...
    lowerBound = smallestIndex;
}

void higherBoundSearch(vector<int> nums, int &higherBound, int target) {
    int start = 0;
    int high = nums.size()-1;
    int higherIndex = -1;
    while(start<=high) {
...
vector<int> solve() {
    ...
    int higherBound = INT_MIN;
    lowerBoundSearch(nums, lowerBound, e);
    higherBoundSearch(nums, higherBound, e);
    r.push_back(higherBound-lowerBound+1); // To calculate frequency of 3
