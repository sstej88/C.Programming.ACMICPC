...
bool isPossible(vector<int> nests, int mid, int birds, int &actualResult) {
    int count = 1;
    int firstBird = nests.front();
    for(int i=1; i<nests.size(); i++) {
        if(nests.at(i)-firstBird >= mid) {
            count++;
            firstBird = nests.at(i);
        }
    }
    if(count>=birds) {
        actualResult = mid;
...
int angryBirds(vector<int> nests, int birds, int &actualResult) {
    int min = nests.at(0);
    int max = nests.back();

    int searchSpace = max-min;
    int low = 0;
    int high = searchSpace;
    while(low<=high) {
        int mid = low+(high-low)/2;
        if(isPossible(nests, mid, birds, actualResult)) {
            low = mid+1;
        }
        else {
            high = mid-1;
...
vector<int> solve() {
    ...
    cout<<angryBirds(nests, e, actualResult);
