...
bool checkDivisions(vector<int> &nums, int numberOfPeople, int mid, int &output) {
    int sum = 0;
    int counter = 0;
    for(auto x: nums) {
        sum+=x;
...
int gameOfGreed(vector<int> &nums, int numberOfPeople) {
    ...
    while(start<=end) {
        ...
        if(checkDivisions(nums, numberOfPeople, mid, output)) {
            start = mid+1;
        }
        else {
            end = mid-1;
...
vector<int> solve() {
    ...
    r.push_back(gameOfGreed(coins, e));
