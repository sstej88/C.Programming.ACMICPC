...
void counter(vector<int> &nums, int low1, int high1, int low2, int high2, int &count, vector<int> &output) {
    if(low1>high1) {
        for(int i=low2; i<=high2; i++) {
            output.push_back(nums.at(i));
        }
    ...
    if(nums.at(low1)>=nums.at(low2)) {
        count = count + high1 - low1 + 1;
        output.push_back(nums.at(low2));
        counter(nums, low1, high1, low2+1, high2, count, output);
    }
    if(nums.at(low2)>=nums.at(low1)) {
        output.push_back(nums.at(low1));
        counter(nums, low1+1, high1, low2, high2, count, output);
    }
}

void countInversions(vector<int> &nums, int low, int high, int &count) {
    if(low>=high) {
        return;
    }
    else {
        int mid = low+(high-low)/2;
        countInversions(nums, low, mid, count);
        countInversions(nums, mid+1, high, count);

        vector<int> output;
        counter(nums, low, mid, mid+1, high, count, output);

        int index = 0;
        for(int i=low; i<=high; i++) {
            nums.at(i) = output.at(index);
            index++;
        }
    }
}
...
vector<int> solve() {
    ...
    int count = 0;
    countInversions(nums, 0, nums.size()-1, count);
