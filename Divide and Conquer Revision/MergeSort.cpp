...
void merge(vector<int> &nums, int low1, int high1, int low2, int high2, vector<int> &output) {
    if(low1>high1) {
    ...
    if(nums.at(low1)>=nums.at(low2)) {
        ...
        merge(nums, low1, high1, low2+1, high2, output);
    }
    else {
        output.push_back(nums.at(low1));
        merge(nums, low1+1, high1, low2, high2, output);
    }
}
...
void mergeSort(vector<int> &nums, int low, int high) {
    ...
    vector<int> output;
    merge(nums, low, mid, mid+1, high, output);
