...
void mergeTwoSortedArrays(vector<int> &nums, int low1, int high1, int low2, int high2, vector<int> &aux) {
    if(low1>high1) {
    ...
    if(nums.at(low1)>=nums.at(low2)) {
        aux.push_back(nums.at(low2));
        mergeTwoSortedArrays(nums, low1, high1, low2+1, high2, aux);
    }
    if(nums.at(low2)>=nums.at(low1)) {
        aux.push_back(nums.at(low1));
        mergeTwoSortedArrays(nums, low1+1, high1, low2, high2, aux);
    }
}
...
void mergeSort(vector<int> &nums, int low, int high) {
    if(low>=high) {
        ...
        vector<int> aux;
        mergeTwoSortedArrays(nums, low, mid, mid+1, high, aux);

        int index = 0;
        for(int i = low; i<=high; i++) {
            nums.at(i) = aux.at(index);
            index++;
        }
    }
}
