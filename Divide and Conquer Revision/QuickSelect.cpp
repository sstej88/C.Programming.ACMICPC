...
int quickSelect(vector<int> &nums, int low, int high, int required) {
    int pivot = high;
    int j = low-1;
    ...
    else if(pivot<required) {
        return quickSelect(nums, pivot+1, high, required);
    }
    else {
        return quickSelect(nums, low, pivot-1, required);
    }
}
...
vector<int> solve() {
    ...
    cin>>n;
    cout<<quickSelect(nums, 0, nums.size()-1, --n)<<endl;
