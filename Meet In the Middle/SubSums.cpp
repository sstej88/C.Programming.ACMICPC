...
void findSubsetsSum(vector<int> nums, int low, int high, vector<int> &sumArray, vector<int> output, int sum, vector< vector<int> > &subsets) {
    if(low>=high) {
        return;
    }
    findSubsetsSum(nums, low+1, high, sumArray, output, sum, subsets);
    output.push_back(nums.at(low));
    sum+=output.back();
...
void subsetsSum(vector<int> &nums, int target) {
    int low = 0;
    int high = nums.size()-1;
    int mid = low+(high-low)/2;

    vector< vector<int> > leftSubsets;
    vector< vector<int> > rightSubsets;

    vector<int> leftSubsetsSum;
    vector<int> leftSubset;
    findSubsetsSum(nums, low, mid+1, leftSubsetsSum, leftSubset, 0, leftSubsets);
    vector<int> rightSubsetSum;
