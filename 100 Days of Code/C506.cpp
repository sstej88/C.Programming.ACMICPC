...
void getSubStrings(string s, int i, set< vector<char> > &result, vector<char> r) {
    if(i==s.size()) {
        return;
    }
    r.push_back(s.at(i));
    result.insert(r);
    getSubStrings(s, i+1, result, r);
}

bool isPalindrome(vector<char> r) {
    int i = 0;
    int j = r.size()-1;
    while(i<=j) {
        if(r.at(i)!=r.at(j)) {
