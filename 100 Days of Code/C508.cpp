...
int coinsFunc(vector<int> &coins, int amount, int total, unordered_map<int, int> &dp) {
    if(amount==0) {
        return total;
    }
    if(amount<0) {
    ...
    for(auto x: coins) {
        pq.push(coinsFunc(coins, amount-x, total+1, dp));
    }
    return pq.top();
}
...
vector<int> solve() {
    ...
    if(amount==0) cout<<0<<endl;
        ...
        priority_queue<int, vector<int>, greater<int> > pq;
        for(auto x: nums) {
            pq.push(coinsFunc(nums, amount-x, total, dp));
        }
        if(pq.top()==INT_MAX) cout<<-1<<endl;
