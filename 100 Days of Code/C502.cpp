...
int findCost(vector<int> cost, unordered_map<int, int> &dp, int i) {
    if(i==cost.size()) {
        return 0;
    }
    if(i>cost.size()) {
    ...
    if(dp.count(i+1)) {
        ...
        dp[i+2] = a = findCost(cost, dp, i+2);
    }
    if(dp.count(i+2)) {
        b = dp[i+2];
    }
    else {
        dp[i+1] = b = findCost(cost, dp, i+2);
    }
    if(dp.count(i)) {
        c = dp[i];
    }
    else {
        dp[i] = c = cost[i]+min(findCost(cost, dp, i+1), findCost(cost, dp, i+2));
    }
    return c;
}
