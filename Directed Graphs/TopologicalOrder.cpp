...
class DAG {
    ...
    void addEdge(string a, string b, bool isDirected = true) {
        graph[a]->connectedNodes.push_back(b);
        if(!isDirected) {
            graph[b]->connectedNodes.push_back(a);
        }
    }
    ...
    void topologicalSort(string a) {
        if(visited.size()==numberOfNodes) {
            return;
        }
        visited[a] = true;
        for(auto x: graph[a]->connectedNodes) {
            if(visited.count(x)==0) {
                visited[x] = true;
                topologicalSort(x);
            }
