...
bool checkAtPositionJ(vector<string> &board, int i, int j) {
    // check upper column
    for(int row = 0; row<=i; row++) {
        if(board.at(row).at(j)=='Q'){
            return false;
        }
    }
    // Right Diagonal
    ...
    while(col<board.size()&&row>=0) {
        if(board.at(row).at(col)=='Q') return false;
        row--;
        col++;
    }
    // Left Diagonal
...
bool solveNQueens(vector<string> &board, int n, int i) {
    if(i==n) {

        cout<<"N Queens Arrangement: "<<endl;
        for(auto x: board) {
            cout<<x<<endl;
        }
        cout<<endl;
        ...
        for(int j=0; j<n; j++) {
            ...
            bool nextRowOutput = true;
            if(checkAtPositionJ(board, i, j)) { // Check Whether I can put Queen or Not
                board.at(i).at(j) = 'Q';
                placedQueen = true;
                if(placedQueen) {
                    nextRowOutput = solveNQueens(board, n, i+1);
