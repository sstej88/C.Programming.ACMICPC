...
void brackets(int n, vector<string> &output, string subset, int open, int close, int i) {
    if(i==2*n) {
        cout<<subset<<endl;
        output.push_back(subset);
        return;
        ...
        if(open<n) {
            subset+="(";
            brackets(n, output, subset, open+1, close, i+1);
            subset.pop_back();
        }
        if(close<open) {
            subset+=")";
            brackets(n, output, subset, open, close+1, i+1);
            subset.pop_back();
        }
    }
}
...
vector<int> solve() {
    ...
    string subset;
    brackets(n, output, subset, 0, 0, 0);
