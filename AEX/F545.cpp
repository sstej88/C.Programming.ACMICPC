...
vector<int> sortedSquaredArray(vector<int> array) {
  ...
  vector<int> result;
  int posIndex = array.size()-1;
  int negIndex = 0;
  while(negIndex<=array.size()-1 && array.at(negIndex)<0) {
    negIndex++;
  }
  while(posIndex>=0 && array.at(posIndex)>=0) {
    posIndex--;
  }
  negIndex--;
  posIndex++;
  ...
  while(posIndex<array.size() && negIndex>-1) {
    cout<<array[posIndex]<<" "<<-array[negIndex]<<endl;
    if(array[posIndex]<=-array[negIndex]) {
      actualArray.push_back(array[posIndex]*array[posIndex]);
      posIndex++;
    }
    else {
      actualArray.push_back(-array[negIndex]*-array[negIndex]);
      negIndex--;
    }
  }
  while(posIndex<array.size()) {
    actualArray.push_back(array[posIndex]*array[posIndex]);
    posIndex++;
  }
  while(negIndex>-1) {
