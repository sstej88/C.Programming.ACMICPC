...
void bst(BST *tree, int target, int &min) {
  if(tree==NULL) {
    return;
  }
  ...
  int checkDiff = target-min;
  if(checkDiff<0) checkDiff = -checkDiff;
  if(difference < checkDiff) min = tree->value;
  
  if(target==tree->value) {
    return;
  }
  else if(target>tree->value) {
    bst(tree->right, target, min);
  }
  else {
    bst(tree->left, target, min);
  }
}
