...
class MSTGraph {
    ...
    MSTGraph(int n) {
        ...
        union_set = new int[numberOfNodes];
        for(int i=0; i<numberOfNodes; i++) {
            union_set[i] = -1;
        }
    }
    ...
    void displayEdges(priority_queue<pair<int, pair<int, int> >, vector<pair<int, pair<int, int> > >, greater<pair<int, pair<int, int> > > > tempPq) {
        while(!tempPq.empty()) {
            pair<int, pair<int, int> > p = tempPq.top();
    ...
    int find(int i) {
        ...
        return find(union_set[i]);
    }

    vector<pair<int, pair<int, int> > > includingEdges;
    void kruskalsAlgo() {
        while(!pq.empty()) {
            pair<int, pair<int, int> > p = pq.top();
            int s1 = find(p.second.first);
            int s2 = find(p.second.second);
            if(s1!=s2) {
                includingEdges.push_back(p);
                union_set[s2] = s1;
            }
            pq.pop();
        }
...
vector<int> solve() {
    ...
    g.kruskalsAlgo();
    int minimumWeight = 0;
    for(auto x: g.includingEdges) {
        minimumWeight += x.first;
        cout<<x.first<<" "<<x.second.first<<" - "<<x.second.second<<endl;
    }
    cout<<endl<<minimumWeight<<endl;
