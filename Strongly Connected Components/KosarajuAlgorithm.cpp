...
class Graph {
    ...
    stack<int> dfsResult; // I need the reverse of dfs result, so I used stack. We can also perform normal dfs and reverse the output of dfs.
    void dfs(int i) {
    ...
    void dfsOnReverseGraph(int i, int component) {
        if(visited.size()==numberOfNodes) {
            return;
        }
        visited[i] = true;
...
vector<int> solve() {
    ...
    g.visited.clear();
    int component = 0;
    while(!g.dfsResult.empty()) {
        if(g.visited.count(g.dfsResult.top())==0) {
            g.dfsOnReverseGraph(g.dfsResult.top(), component);
            component++;
        }
        g.dfsResult.pop();
    }
