...
class SpGraph {
    ...
    SpGraph(int n, int source) {
        this->numberOfNodes = n;
        l = new list< pair<int, int> >[numberOfNodes];
        result = new int[numberOfNodes];
        for(int i=0; i<numberOfNodes; i++) {
            result[i] = INT_MAX;
        }
        result[source] = 0;
    ...
    void dijkstraAlgorithm(int node, int distance) {
        if(finalized.size()==numberOfNodes) {
            return;
        }
        finalized[node] = true;
        for(auto x: l[node]) {
            if(finalized.count(x.second)==0) {
                x.first += distance;
                // cout<<x.second<<" "<<x.first<<endl;
                result[x.second] = min(result[x.second], x.first);
                pq.push(x);
            }
        }
        pair<int, int> activeNode = pq.top();
        pq.pop();
        dijkstraAlgorithm(activeNode.second, activeNode.first);
    }
};
...
vector<int> solve() {
    ...
    g.dijkstraAlgorithm(1, 0);
    for(int i=0; i<g.numberOfNodes; i++) {
        cout<<i<<" - "<<g.result[i]<<endl;
    }
