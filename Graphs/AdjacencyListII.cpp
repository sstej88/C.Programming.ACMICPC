...
class Graph {
    public:
    int numberOfNodes;
    unordered_map<string, Node *> adjacencyMatrix;

    Graph(vector<string> cities) {
        numberOfNodes = cities.size();
        for(auto city: cities) {
            adjacencyMatrix[city] = new Node(city);
        }
    }
    void addEdge(string source, string destination, bool unDirected = false) {
        adjacencyMatrix[source]->connectedNodes.push_back(destination);
        if(unDirected) {
            adjacencyMatrix[destination]->connectedNodes.push_back(source);
        }
    }
    void displayGraph() {
        for(auto x: adjacencyMatrix) {
            cout<<x.first<<" -> ";
            for(auto y: x.second->connectedNodes) {
                cout<<y<<" ";
            }
            cout<<endl;
        }
...
vector<string> solve() {
    vector<string> nums;
    nums.push_back("Delhi");
    nums.push_back("London");
    nums.push_back("Paris");
    nums.push_back("New York");
