...
int fibonacciSeries(int n, unordered_map<int, int> &ump) {
    if(n==0 || n==1) return n;
    int a = ump[n-1] = ump.count(n-1)?ump[n-1]:fibonacciSeries(n-1, ump);
    int b = ump[n-2] = ump.count(n-2)?ump[n-2]:fibonacciSeries(n-2, ump);
    ump[n] = a+b;
    return ump[n];
}
...
vector<int> solve() {
    ...
    nums.push_back(fibonacciSeries(6, ump));
