...
int arrayJumps(int i, int numberOfJumps, vector<int> nums, vector<int> &dynamicProgramming) {
    if(i>=nums.size()) {
        return INT_MAX;
    }
    if(i==nums.size()-1) {
    ...
    // if(dynamicProgramming.at(i)<INT_MAX) {
    //     cout<<dynamicProgramming.at(i)<<" - "<<i<<endl;
    //     return dynamicProgramming.at(i);
    // }
    
    priority_queue<int, vector<int>, greater<int> > pq;
    for(int j=i+1; j<=i+nums.at(i); j++) {
        pq.push(arrayJumps(j, numberOfJumps+1, nums, dynamicProgramming));
    }
    dynamicProgramming.at(i) = pq.top();
    return pq.top();
}
...
vector<int> solve() {
    ...
    int n, e;
    cin>>n;
    vector<int> dynamicProgramming(n, INT_MAX);
    while(n--) {
        cin>>e;
        nums.push_back(e);
    }
    ...
    int i = 0;
    int numberOfJumps = 0;
    cout<<arrayJumps(i, numberOfJumps, nums, dynamicProgramming)<<endl;

    for(auto x: dynamicProgramming) {
        cout<<x<<" ";
    }
    cout<<endl;
