...
void findSubsets(string &digits, string subset, vector<string> &result, int i) {
    if(i==digits.length()) {
        result.push_back(subset);
        return;
    }
    else {
        findSubsets(digits, subset+digits.at(i), result, i+3);
        findSubsets(digits, subset, result, i+3);
    }
}
...
vector<string> letterCombinations(string digits) {
    ...
    findSubsets(actual, subset, result, 0);
    result.push_back("");
    return result;
}
