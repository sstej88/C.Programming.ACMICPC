...
void mergeAlgo(vector<int> &nums1, int i, int k, vector<int> &nums2, int j, int l, vector<int> &output) {
    if(i>k) {
        for(int index=j; index<=l; index++) {
            output.push_back(nums2.at(index));
        }
        return;
    }
    else if(j>l) {
        for(int index=i; index<=k; index++) {
            output.push_back(nums1.at(index));
        }
        return;
    }
    else {
        if(nums1.at(i)<=nums2.at(j)) {
            ...
            mergeAlgo(nums1, i+1, k, nums2, j, l, output);
        }
        else if(nums2.at(j)<=nums1.at(i)) {
            output.push_back(nums2.at(j));
            mergeAlgo(nums1, i, k, nums2, j+1, l, output);
        }
    }
}
void merge(vector<int>& nums1, int m, vector<int>& nums2, int n) {
    vector<int> output;
    mergeAlgo(nums1, 0, m-1, nums2, 0, n-1, output);
    nums1.clear();
    nums1 = output;
}
...
vector<int> solve() {
    ...
    vector<int> nums1(nums.begin(), nums.begin()+e);
    for(auto x: nums1) {
        cout<<x<<" ";
    }
    cout<<endl;
