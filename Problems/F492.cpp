...
public:
    ...
    int find(int a) {
        int parent = union_arr[a];
        if(parent==-1) return a;
        return union_arr[a] = find(parent);
    }
    bool joinEdge(int a, int b) {
        int parent1 = find(a);
        int parent2 = find(b);
        ...
        if(parent1==parent2) {
            if(parent1==-1) {
                if(parent1<parent2) {
                    union_arr[parent1] = parent2;
                }
                else {
                    union_arr[parent2] = parent1;
                }
            }
            else {
...
vector<int> solve() {
    ...
    cout<<s.canFinish(numberOfNodes, prereq)<<endl;
