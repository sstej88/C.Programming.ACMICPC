...
void dfs(Node* node, unordered_map<int, bool> &visited, vector< pair<int, vector<Node*> > > &adjacencyList) {
    if(node!=NULL) {
        if(visited.count(node->val)==0) {
            adjacencyList.push_back(make_pair(node->val, node->neighbors));
            visited[node->val] = true;
            for(auto x: node->neighbors) {
                if(visited.count(x->val)==0) {
                    dfs(x, visited, adjacencyList);
                }
            }
        }
    }
...
Node* cloneGraph(Node* node) {
    unordered_map<int, bool> visited;
    vector< pair<int, vector<Node*> > > adjacencyList;
    dfs(node, visited, adjacencyList);
    
    Node **root = new Node*[adjacencyList.size()+1];
    for(auto x: adjacencyList) {
        root[x.first] = new Node(x.first, x.second);
    }
