...
class Solution {
public:
    void applyBFS(TreeNode* root, vector<vector<int>> &result, unordered_map<int, vector<int>> &value, int level) {
        if(root==NULL) {
            return;
        }
        level = level+1;
        value[level].push_back(root->val);
        applyBFS(root->left, result, value, level);
        applyBFS(root->right, result, value, level);
    }
    vector<vector<int>> levelOrder(TreeNode* root) {
        vector<vector<int>> result;
        unordered_map<int, vector<int>> value;
        int depth = 0;
        applyBFS(root, result, value, 0);
        depth = value.size();
        // for(auto x: value) {
        //     cout<<x.first<<" - ";
        //     for(auto y: x.second) {
